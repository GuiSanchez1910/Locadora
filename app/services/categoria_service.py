...
def atualizar(categoria_id: int, dados: dict) -> Categoria:
    ...
    if "nome" in dados:
        _garantir_nome_disponivel(dados["nome"], categoria_id)
...
def _garantir_nome_disponivel(nome: str, categoria_id: int | None = None) -> None:
    stmt = db.select(Categoria).where(Categoria.nome == nome)
    if categoria_id is not None:
        stmt = stmt.where(Categoria.id != categoria_id)

    if db.session.scalar(stmt) is not None:
        raise RegraDeNegocio("Nome de categoria já cadastrado.")